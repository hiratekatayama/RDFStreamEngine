...
public class CQELSEngine {
	...
	public void send(Node graph, Node s, Node p, Node o) {
		EnQuad enquad = new EnQuad(encode(graph), encode(s), encode(p), encode(o));
		//System.out.println("context dictionary getId s: " + this.context.dictionary().getAllocateNodeId(s).getId() + " node: " + s);
		//System.out.println("context dictionary getId o: " + this.context.dictionary().getAllocateNodeId(o).getId() + " node: " + o);
		this.provider.getEPRuntime().sendEvent(enquad);
	}
